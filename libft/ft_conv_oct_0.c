...
{
	(flags->hash == 1) ? flags->zero-- : 0;
	(flags->hash == 1) && (!flags->display) ? ft_putstr("0") : 0;
					...
					long long local_pa)
{
	...
	if (index == 0 && flags->hash == 1)
	{
		if (flags->preci == -1 || (flags->preci == 0 && local_pa != 0))
			(!flags->display) ? ft_putstr("0") : 0;
		else if (local_pa == 0)
			flags->hash = 0;
		else if (local_pa > 0 && flags->hash)
			(!flags->display) ? ft_putstr("0") : 0;
	}
	(flags->hash == 1) ? flags->spaces_count++ : 0;
}
